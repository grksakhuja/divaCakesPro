...
var transporter = nodemailer.createTransport({
  ...
  auth: {
    ...
    // Brevo SMTP login (email)
    pass: process.env.BREVO_SMTP_PASS
    // Brevo SMTP password
  }
});
var createAdminEmailTemplate = (order) => {
  const totalPrice = (order.totalPrice / 100).toFixed(2);
  const orderDate = new Date(order.orderDate).toLocaleDateString();
  return {
    subject: `\u{1F382} New Cake Order #${order.id} from ${order.customerName}`,
    html: `
...
var createCustomerEmailTemplate = (order) => {
  const totalPrice = (order.totalPrice / 100).toFixed(2);
  const orderDate = new Date(order.orderDate).toLocaleDateString();
  const estimatedDays = order.deliveryMethod === "pickup" ? "2-3" : "3-4";
  return {
    subject: `\u{1F382} Thank You for Your Cake Order #${order.id}`,
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #eee; border-radius: 8px;">
